...
We are going to start with an example which is connected to a more involved duality called the Stone duality, which will be our second example. For our first example we will like to elucidate its schizophrenic nature as it ties directly to the generality we have learned, so that we can start to get a sense for in what direct way we can lift Hom-sets.
...
In this part we assume basic knowledge about topological spaces and frames. We will only use the definitions of objects and morphisms in $\Top$ and $\Frm$.  Our reference text when it comes to any discussion about posets, lattices, frames, or locales, is \emph{Johnstone}'s \emph{Stone Spaces}. 
...
On the other hand finite Boolean algebras are atomic, since every non-zero element is bounded below by finitely many elements, so there exists at least one minimal one. 

Therefore ultrafilters in $\Boolf$ correspond exactly to the atoms, and one observes that every element is the join of atoms below it [Joh]. Since for distinct $b$ and $b'$, an atom below $b$ but not $b'$ generates an ultrafilter that separates them, the counit $B \to \mathcal{P}(\Ult(B))$ that sends $b \mapsto \{U \in \Ult(B) \lvert b \in U\}$ recovers  $b$ uniquely, and so it is an isomorphism. One needs only to check that the counit is indeed a Boolean algebra homomorphism, but this is straightforward.
%
%
...
On the other hand, for a finite Boolean algebra $A$ we have the following bijection \begin{align*}
...
In fact, the above map generalizes to a bijection on distributive lattices $\phi': \prim(A) \to \DLat(A, \mathbb{2})$, where $\prim(A)$ are the prime filters of $A$. We call any map $f \in \mathcal{L}(A, \mathbb{2})$ \textbf{\emph{a point}}, for any subcategory $\mathcal{L}$ of $\DLat$.

Obviously the map $f \mapsto f^{-1}(1)$ is an inverse, so we only check that these are well-defined. 

The proof for $\phi'$ is straightforward and boils down to an equivalence between properties of lattice homomorphisms and prime filter axioms. That is, an order preserving, finite limit preserving, and finite colimit preserving morphism corresponds to a prime, upwards closed, sub meet-semilattice.

Now when restricting to Boolean algebras (note that $\Bool$ is a \emph{full} subcategory of $\DLat$), prime filters correspond bijectively to ultrafilters, which, given the axioms of being a filter, will additionally  require that $A \in F$ if and only if $\forall B \in F: B \cap A \neq \emptyset$. 

The property that a filter $F$ is prime is equivalent to $F$ being given above by $f^{-1}(1)$ for $f \in \DLat(A, \mathbb{2})$ which is further equivalent to the fact that its complement $I:= f^{-1}(0)$ is an ideal [Joh]. \emph{We write $\DLat$ here instead of $\Bool$ to highlight that we do not use complement preservation for this direction}.
...
Ultimately, what a point map  $A \stackrel{f}{\rightleftarrows} \mathbb{2}$ represents is the question of which subsets of our lattice $A$ are compatible with our desired structure restrictions. 
...
$\textbf{Set of points}$& $\textbf{Type of filter}$\\
 \hline
 \hline 
 $\DLat(A, \mathbb{2})$   &prime filters \\
 \hline
 $\Frm(A, \mathbb{2})$ &completely prime filters \\
 \hline 
 $\Bool(A, \mathbb{2})$    &ultrafilters \\
 \hline
\end{tabular}\]

We will only show initiality of evaluation maps for the general Stone duality.
...
\subsection{Concatenating subdualities of $\Top \rightleftarrows \Frm^{\op}$ with the $\CohLoc \rightleftarrows \DLat^{\op}$ duality}
In the following we will want to give a general overview of how we obtain the Stone duality.

Before we define it we first focus on the following dual adjunction
...
\end{tikzcd}\]
 ...
 which is  the duality of our leading example. This is indeed given by $\pt(A) := \Frm(A, \mathbb{2})$, and $\Top(X, \mathbb{S}) =: \Omega(X)$, which is the frame of opens on $X$, since a continuous map into the Sierpiński space is  uniquely determined by the preimage of $\{1\} \subset \mathbb{S}$, which corresponds to a unique open set of $X$. 
...
Given $A, B \in \CohLoc$, we know that any lattice homomorphism $K(A) \to K(B)$ extends to a frame homomorphism $A \to B$ via the free-forget adjunction (\textcolor{red}{TODO}: find source for this), however the converse is not true, frame homomorphisms do not necessarily preserve finiteness.

In topological terms, the preimage of compact open subsets under continuous maps is not necessarily compact.
...
In order to answer these questions, we will first show that the composite subduality $\CohTop \rightleftarrows \DLat^{\op}$ has a schizophrenic object, and then see what happens when we restrict to the Stone duality.
