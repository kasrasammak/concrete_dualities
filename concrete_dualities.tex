...
\title{\textsc{On the Structure of \\ Concrete Dualities \\via  Examples }
\\[3em]
...
Concrete dualities appear throughout mathematics in forms that seem unrelated at first glance: between topological spaces and certain classes of lattices, between commutative rings and sets with a group action, between rings and affine schemes. A particularly introductory example is the classical self-duality of finite-dimensional vector spaces via the dual space construction. A common feature of these dualities is that they arise from a dual adjunction in which a single object carries compatible structure on both sides. Such an object — often called \emph{schizophrenic} or \emph{dualizing} — induces a particularly well behaved dual adjunction between the associated categories of structured sets, and by restriction to suitable subcategories an equivalence. The guiding theme of this thesis is that many familiar dualities can be understood uniformly through this mechanism.
...
At the same time, we hope to gesture toward the underlying philosophical insight: dualities of this kind allow topological or geometric intuition to be translated into algebraic or logical intuition (and vice versa), revealing that the two viewpoints often describe the same underlying structure. By the end of the thesis, the reader should be able to recognize the mechanism behind a concrete duality: how a schizophrenic object induces a well-behaved dual adjunction and organizes structure that underlies classical dualities across mathematics. To understand why these individual examples genuinely instantiate this mechanism constitutes the key takeaway.
...
\section{Free Objects and the Free-Forget Adjunction}\label{subsec: free-objects}
...
Given some $\mathcal{A}$-morphism $f: A\to A'$, consider the map $Uf: UA \to UA'$. More explicitly, this is the map \begin{align*}
 	...
 	\t x \mapsto& f\circ \t x.
 \end{align*}
Though $\t x$ is a map, we may understand it as per \ref{subsec: free-objects} as a set element $x \in U(A)$. Therefore we use  the notation $[f]: [A] \to [A']$, which reinforces the intuition that regardless of the underlying functor, this is just the underlying set map.
...
Furthermore we see that \begin{align}\label{3}
	e_{SB, t} \circ \eta_B = t \ \ \text{ for all } t \in [SB]= \HomB(B, \t B).
\end{align}
Since $[\eta_B]$ lifts for all $B$, we may verify naturality on underlying-set maps; we verify that given a $\mathcal{B}$-morphism $f: B \to B'$, that we have $ [\eta_{B'}]\circ (f \circ -) = [TSf] \circ [\eta_B]$, or in other words: \begin{align}\label{4}
	d_{B', f \circ y} = [TSf] \circ d_{B,y}.
\end{align} 
Remember that the left action of $[TSf]$ on $d_{B,y}$ is a right action of $Sf$ on $d_{B,y}$. But the underlying map of $d_{B,y} \circ Sf$ is $\sigma \psi_{B,y} \circ \HomB(f, \t B)$ which is, up to the bijection $\sigma$, just the evaluation map of a morphism in $\HomB(B, \t B)$ at some $y \in [B]$ precomposed with $f \in \HomB(B, B')$, which yields the evaluation map of a morphism in $\HomB(B',\t B)$ at $f \circ y \in [B']$. In other words $[d_{B,y}] [Sf] = [\sigma \psi_{B', f \circ y}] = [d_{B', f \circ y}]$, which is clearly the underlying set map of $d_{B', f \circ y}$, giving us \ref{4} by uniqueness of the lift. 



The definition of $S$ gives us that $[S\eta_B][\epsilon_{SB}](t) = \HomB(\eta_B, \t B)(e_{SB, t})$ and by \ref{3} we have $\HomB(\eta_B, \t B)(e_{SB, t}) = e_{SB,t} \circ \eta_B = t$. Since $U$ is faithful, any map $[Sf] \in \Set([SB'], [SB])$ is the underlying-set map to a unique map $Sf \in \HomA(SB', SB)$, and we deduce that $[S\eta_B][\epsilon_{SB}] = 1_{[SB]}$ is the underlying set map of the triangle identity $S\eta_B \circ \epsilon_{SB} = 1_{SB}$.
...
Firstly, one may use the triple $(\t A, \tau, \t B)$ to induce a natural dual adjunction $(S,T)$ on the concrete categories $(\mathcal{A}, U)$ and $(\mathcal{B},V)$. Such a method requires additional assumptions on $(\mathcal{A},U)$ and $(\mathcal{B},V)$ which we will not discuss here, nor will this method appear in our examples. For more details, see section 1-D of \cite{MR1147921}. 

The second method is the one which we will later see in action in one of our examples, which is to restrict our  adjunction to full subcategories under which we have a natural equivalence. Though we note that this method may require additional assumptions.
...
If the desired lifts exist, then by Theorem \ref{thm: 4.1} we will have the following adjunction
\[\begin{tikzcd}
	{\mathcal{A}} & \bot & {\mathcal{B}^{\op}}
...
\chapter{Examples}
The rest of our thesis shall be devoted to an analysis of several specific examples (and non-examples) of concrete dualities. Our goal is not only to show that our candidates are indeed schizophrenic objects (or why they are not), but to build intuition for the schizophrenic object through a careful analysis of the induced adjunctions. 
...
We will subsequently connect this example to a duality called the Stone duality, by which we plan to give concrete descriptions of these $\Hom$-set functors, for which we had just given a more general description using only universal properties and  $\Hom$-sets. Though we do lead with the general into the concrete, we may otherwise view our elucidation of the concrete as the mental abstraction, since we show the actual mechanics behind why we can think of these concrete objects as lifts of $\Hom$-sets. 

This will then lend itself to the intuition of the following examples.
...
This example considers a dual adjunction between the category of topological spaces with continuous maps between them, denoted $\Top$ and the category of frames with frame homomorphisms between them, denoted $\Frm$. We assume basic knowledge about these categories; for now we will only use the definitions of their respective objects and morphisms.  Our reference text when it comes to any discussion about posets, lattices, frames, or locales, is \emph{Johnstone}'s \emph{Stone Spaces} \cite{zbMATH03940199}. 
...
\end{align*}
In this setting  the respective representable objects are $A_0 = \{pt\}$ and $B_0 = \hat b$, where $\hat b$ is here  notation for the free frame generated by one object $b$, i.e., the free 3-chain $\{\bot \leq b \leq \top\}$. 

It is completely straightforward to check that $\{pt\}$ satisfies the universal property. As a complete lattice a frame must include bottom and top elements $\bot$ and $\top$, and furthermore finite limit and colimit preservation of a frame homomorphism $p$ ensures that $p(\bot) = \bot$ and $p(\top) = \top$. With this it is straightforward to check that $\{\bot \leq b \leq \top\}$ satisfies the universal property (\emph{remember that to check this we always start with a set injection $\{pt\} \hookrightarrow [F(\{pt\})]$, where $\{pt\}$ gets sent to our free generator, in this case $b$}).

Therefore we have concrete categories $(\Top, U)$ and $(\Frm, V)$ with $U(-) = \Top(\{pt\}, -)$ and $V(-) = \Frm(\hat b, -)$.
...
For the forward direction we have \begin{align*}
	e_{A,x}(u \leq v) = \big(e_{A,x}(u) \leq e_{A,x}(v)\big) = \big(u(x) \leq v(x)\big),
...
With this we are done. We do not need to prove triangle equalities, we do not need to prove universality. We only prove that our candidate is schizophrenic, and Theorem \ref{thm: 4.1} gives us the adjunction.
...
\subsection{The $\FinSet \rightleftarrows \Boolf^{\op}$ Duality}\label{sec:finite-stone-duality}
...
Here we consider a duality between the concrete category $(\FinSet, U)$ of finite sets and set maps between them, with the representable underlying functor being $U(-) = \Set(\{pt\},-)$ (the universal property follows by definition), and the concrete category $(\Boolf, V)$ of finite Boolean algebras and Boolean algebra homomorphisms between them, whose underlying functor we describe in the following.

Consider that the free Boolean algebra on one generator $b$ is the set $\diamondsuit : =\{\bot, b, \neg b, \top \}$. This is analogous to the frame example, we only remark that  our generator needs to induce complements, since Boolean algebras are complete under complements. To see the universal property, we notice that Boolean algebra homomorphisms are complement preserving, so that they are completely determined by where they send the free generator $b$. Therefore we have $V(-) = \Boolf(\diamondsuit, -)$. 


Let $\Ult(-)$ be the functor which sends a finite Boolean algebra $B$ to the set $\Ult(B)$ of ultrafilters on $B$ and Boolean algebra homomorphisms $B \stackrel{h}{\rightarrow} B'$ to the usual inverse image morphism $\Ult(B') \stackrel{h^{-1}}{\rightarrow}\Ult(B)$. A straightforward computation shows that $h^{-1}$ is well-defined.
...
For this section we remark that we only want to see that a lift of $\Hom$-sets exists, as we will show initiality later when we get to the general $\Stone$ duality. We check for existence by showing that the functors we gave serve as the desired lifts. 
...
\begin{lemma}\label{lem: prime-dlat-bijection} The following map, where $\prim(-)$ sends a distributive lattice to its set of prime filters, is a bijection:
\begin{align*}
...
\end{tikzcd}\]
 ...
 which is  the duality of our leading example; we simply remind the reader that $\Loc := \Frm^{\op}$ by definition. This is indeed given by $\pt(A) := \Frm(A, \mathbb{2})$, and $\Top(X, \mathbb{S}) =: \Omega(X)$, which is the frame of opens on $X$, since a continuous map into the Sierpiński space is  uniquely determined by the pre-image of $\{1\} \subseteq \mathbb{S}$,  corresponding to a unique open set of $X$. 
...
\end{align*}
In a sober topological space, points of $\Omega(X)$ correspond to prime elements, i.e. elements generating prime principal ideals, whose complements are the irreducible closed subsets of $X$, where sobriety is just the condition that singletons are the only irreducible closed subsets of $X$ \cite{zbMATH03940199}.
...
\\
\begin{definition}\label{def: finite}
...
However, the structure of our categories ensures that the inverse maps in $\Sob$ and $\Spat$ that make the unit and counit isomorphisms stay preserved after restriction. That means for any coherent map $B \stackrel{f}{\rightarrow} A$ which is also an isomorphism, the inverse $f^{-1}$ is also coherent. The proof is straight-foward.

Moreover, we can still view these as concrete categories with respect to the same representable objects, since, in particular, $[\{pt\}]$ and $[\hat b]$ are finite sets, so that $\{pt\}$ is compact and $\hat b$ is finite in the sense of \ref{def: finite}, implying  that any continuous map or locale homomorphism out of them, respectively, are coherent.
...
\end{align*}
are well-defined isomorphisms \cite{zbMATH03940199}.
...
It is worth remarking that the reason  $\CohTop \rightleftarrows \CohLoc$ was not natural was because $\CohTop(X, \mathbb{S}) $ gave us $K\Omega(X)$ and not $\Omega(X)$. So intuitively, one could think of this composition as precisely how one preserves the schizophrenic object.

On the one hand, Lemma \ref{lem: prime-dlat-bijection} shows us that $\DLat(B, \mathbb{2})$ lifts to $\prim(B)$. Indeed, $\Spec(B) = \Frm(\Idl(B), \mathbb{2})$, which gives us completely prime filters of $\Idl(B)$, and these correspond precisely to $\prim(B)$, which follows from 3.4 in \cite{zbMATH03940199}. 
...
To obtain the Stone duality, we will once again restrict the above equivalence. 
...
In the following we will show that the Stone duality is a natural dual adjunction. 

Recall the finite setting of \ref{sec:finite-stone-duality}. Indeed, to show that they were concrete categories, we did not use anything about their finite nature, so that our respective underlying representable functors are the same, with $\{pt\}$ and $\diamondsuit$ as the representable objects, respectively.

To determine if the triple we obtained $(\mathbb{2}, 1_{\mathbb{2}}, \mathbb{2})$ is a schizophrenic object, we ask ourselves: \begin{enumerate}
 	\item\label{item: 1} Can we give the spectrum as a lift of the set of points of a Boolean algebra, i.e. $[\Spec(B)] = \Bool(B, \mathbb{2})$, and is this lift initial?
 	\item\label{item: 2} Can we give the Boolean algebra of clopens of a Stone space as a lift of the set of continuous maps over compact Hausdorff spaces, i.e., does $[\clop(X)] = \Stone(X, \mathbb{2})$, and is this lift initial?
 \end{enumerate}

For \ref{item: 1} recall that Lemma \ref{lem: prime-dlat-bijection} told us that the points of a boolean algebra correspond to its ultrafilters, which are equivalent to prime filters in the Boolean setting.
...
Therefore we have a natural dual equivalence between the categories $\Stone \rightleftarrows \Bool^{\op}$ induced by the schizophrenic object $(\mathbb{2}, 1_{\mathbb{2}},\mathbb{2})$. 
%\[\begin{tikzcd}
%	&& \Loc & \bot & \Top \\
%	&& \vdash && \vdash \\
...
Now we can revisit the finite dual equivalence from the beginning of this section. Any finite Stone space is discrete, since Hausdorffness forces singletons to be clopen: we give singletons by an intersection of all point-separating clopens, i.e. $\{x\} = \bigcap_{y \in X} U_y$ where $x \in U_y$ but $y \not\in U_y$, but this is a finite intersection, so that singletons are themselves clopen. This shows that $\FinStone = \FinSet$, noting that the power-set of finite discrete topological spaces return its clopen sets. It is thus clear that the Stone duality restricts to the finite duality of \ref{sec:finite-stone-duality}. 
...
We can view $(\CAlg,U)$ as a concrete category whose underlying set functor is $U(-) = \CAlg(R[t],-)$. To see that the polynomial ring $R[t]$ in one variable $t$ is the correct representable object, notice that, a $\CAlg$ morphism $R[t] \stackrel{f}{\rightarrow} A$ must commute under $R$, so that it is uniquely determined by which element in  $[A]$ the free variable $t$ gets sent to.

%given the set injection $\{pt\} \stackrel{\psi}{\hookrightarrow} [R[t]]$ which sends $\{pt\} \mapsto t$, it holds that for every $A \in \CAlg$ and set map $\{pt\} \stackrel{\varphi}{\rightarrow} [A]$, there exists a unique $R[t] \stackrel{f}{\rightarrow} A$, such that $\varphi = [f] \circ \psi$,  

On the other side, we define an affine scheme $X \in \Aff$ as a representable functor in the functor category $\Fun(\CAlg, \Set)$.
...
This non-example is included because it is easy to digest and shows that even adjunctions which may appear to contain a schizophrenic object--in this case $(R[t],\tau, \mathbb{A}^1)$--may still fail, strictly speaking, to produce a  concrete duality. Nevertheless, it still lets us see structure of a schizophrenic object on one side of the adjunction, and its failure makes clear which structural properties our dualities truly depend on.
...
The category $\kTop$ is a mono-topological category\footnote{The fact the $\kTop$ is a monotopological category is treated as standard fact in sources \cite{MR1147921} and \cite{DUBUC1971281}, which we will be using  without proof.}, so that we can view $(\kTop, U)$ with underlying functor $U(-) = \kTop(\{pt\}, -)$ as a concrete category. 
...
We may otherwise view the \emph{Kelleyfication} as the right adjoint $k(-)$ to the forgetful functor into $\Top$, which we may also get as a categorical fact, since $\kTop$ is monotopological \cite{adamek1990abstract}. As $\kTop$ is a full subcategory of $\Top$, this puts us in the setting of a coreflective subcategory of $\Top$. 
...
Together with the fact that $\kTop$ is a monotopological category, this then puts us in the setting a category which admits an internal hom-functor.
...
\subsection{Generalized Gelfand Duality}

Our goal here is to obtain a dual adjunction as in \ref{sec: int hom}. For this we define the category $\kAlg$ of complex $k$-algebras, whose objects are $\mathbb{C}$-algebras endowed with a Kelley topology, whose morphisms are the continuous algebra homomorphisms, and whose algebra operations are continuous with respect to $k$-products. 

As the objects are $\mathbb{C}$-algebras, we can view $(\kAlg, V)$ as a concrete category with $V(-) = \kAlg(\mathbb{C}[t],-)$ as the underlying representable functor into $\Set$.

Now we want to check the conditions of the adjunction of  \ref{sec: int hom}. Since $\kAlg \subseteq \kTop$ is a full subcategory and $\mathbb{C}$ is a colimit of its compact Hausdorff subspaces which is moreover a $\mathbb{C}$-algebra, we see that $\mathbb{C} \in \kAlg$, and we call it $\mathbb{C}_a$,  so that $U(\mathbb{C}_a) = \mathbb{C}_s$, by analogous notation. Clearly $\tau = 1_{\mathbb{C}}. $ 
...
\[\begin{tikzcd}
	\kTop &\bot & \kAlg^{\op}
	\arrow["{C_k(-,\mathbb{C}_s)}", bend left, from=1-1, to=1-3]
	\arrow["{\Hom_k(-,\mathbb{C}_a)}", bend left, from=1-3, to=1-1]
\end{tikzcd}\]
...
Note that $C^* \subseteq \kAlg$ is a full subcategory.
 ...
 We could view $C^*$ as a concrete category via the usual underlying-set functor, however this functor is not necessarily representable. For our purposes we will consider the functor $\bigcirc: C^* \to \Set$ which sends any $C^*$-algebra $A$ to it's unit ball $\{ a \in A \  \lvert  \ \lVert a \lVert \leq 1\}$ and each morphism $f$  to its restriction $f_\bigcirc$ to the unit ball of its domain, which is representable as per \cite{MR1147921}.
 
 \subsection{Gelfand-Naimark Duality}
 
 Now we want to see if we can restrict the above functors to suitable subcategories such that our evaluations lift initially. 
 
For any compact Hausdorff space $X$, it holds that $C_k(X,\mathbb{C}_s) = C(X, \mathbb{C}_s)$. The crucial fact is that if $X \in \kHaus$ and $Y$ is a metric space, then $C(X, Y)$ with the compact open topology is metrizable  \cite{bredon1993}. Furthermore, the proofs that metric spaces are first countable and first countable spaces are compactly generated are elementary, so we will omit them.

Therefore the $C^*$-algebra $C^*(X)$ and the function $k$-algebra $C(X) := C_k(X, \mathbb{C}_s)$ coincide algebraically, since continuous functions over compact spaces are bounded in $\mathbb{C}$ and since $X \times_k X = X \times X$. Moreover the topology of $C^*(X)$ for $X$ compact is the compact open topology \cite{engelking1989general},  and therefore they also coincide topologically. That means we have can restrict $C$ to a functor $C: \kHaus \to C^*$.
 
% Furthermore, a metric space is first countable: the collection of open balls with radius $\frac{1}{n}$ forms a countable local basis. Now we want to show that a first countable space is a $k$-space. Consider a set $A$ which is not closed and a point $x \in \overline{A}-A$. By first countability there is a sequence in $A$ that is not in 
...
% \cite{DUBUC1971281}
 Can we restrict $S$ accordingly? For every $C^*$-algebra $A$, it should be clear from \cite{KelleyGeneralTopology} that the space $S(A) = \Hom_k(A, \mathbb{C}_a)$ is compact, and that its topology is that of pointwise convergence.  Indeed \emph{Kelley's} definition of pointwise convergence applied to our situation is exactly 
% the initial topology making the inclusion $S(A) \hookrightarrow \mathbb{C}^A$ continuous, which is just 
 the initial topology making all evaluation maps $\Hom_k(A, \mathbb{C}_a) \stackrel{\ev_{A,a}}{\rightarrow} [\mathbb{C}_c]$ continuous. Furthermore, this is equivalent to the initial topology making the inclusion $S(A) \hookrightarrow \mathbb{C}^A$ continuous, from which Hausdorffness is clear, since the product of Hausdorff spaces is Hausdorff, and the subspace of a Hausdorff space is Hausdorff. 
 
 We call $S: C^* \to \kHaus$ the \emph{Spectrum}-functor, and it follows that the generalized Gelfand-Naimark adjunction restricts to a natural dual adjunction
...
\end{tikzcd}\]
 ...
 We would like to remark that this duality is in fact an equivalence. Albeit interesting, showing this requires machinery that is beyond the scope of this thesis. For details, refer to chapters III. and IV. in \cite{zbMATH03940199}.
...
It should be clear from previous examples that $k[t]$ and $\{pt\}$ are the correct representable objects of our underlying set functors.
...
Explicitly the inhereted $G$-action on $\CAlgk(L,k_s)$ is given by  post-composition, i.e., $g \cdot_{\CAlgk(L,k_s)} \varphi = g \circ \varphi$ for each $L \in \CAlgk$. As $G$ is a group of automorphisms on $k_s$, a $G$-equivariant map 
...
In this thesis we have shown that the concrete dualities examined here all arise from the same mechanism: a dual adjunction induced by a schizophrenic object. Our analysis of the classical examples confirms that, despite their differences, they all fit this general framework. 



%The level of detail varied according to feasibility, and we did not attempt to develop the full surrounding theory within each subfield; the aim was expository rather than exhaustive.

A useful consequence of this perspective is that recognizing a potential schizophrenic object can serve as the primary step toward understanding a duality. In each case, the schizophrenic object serves as an intuitive point of entry, and identifying a suitable candidate, given its systematic nature, is typically far easier than attempting to describe an adjunction ad hoc without first knowing one exists. 

To underline this, notice that it should now be a straightforward exercise to identify the schizophrenic object of the self-duality in the category of finite dimensional vector spaces which we mentioned in the introduction.

The level of detail in this thesis varied according to feasibility; we did not attempt to develop the full surrounding theory within each subfield. However what we did show was that once a candidate object is identified and shown to meet the criteria, the dual adjunction — with its associated structure — follows suit, making it unnecessary to rebuild the theory from the ground up in each new subfield. One can rather find concrete descriptions of the $\Hom$-functors in a top-down fashion, as we did for some of our examples. In this sense, knowing how concrete dualities arise allows us to see familiar constructions not as isolated coincidences but as manifestations of a single underlying framework.
%\begin{enumerate}
