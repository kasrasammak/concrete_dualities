...
Given  $B \in \DLat$ and $a \in B$, let $\atom(B)$ be the set of atoms in $B$ and $\atom(a) := \{x \in \atom(B) \lvert x \leq a \}$. If $B \in \Bool_{\atom}$, one observes that  $\atom(a)=\atom(a')$ implies $a = a'$. Equivalently, every element of an atomic Boolean algebra is the join of atoms below it [Joh]. 
...
Now we will want to compose. Consider the fact that a locale is coherent if and only if it is isomorphic to the locale of ideals of a distributive lattice [Joh]. This gives us a functor $\Idl(-): \DLat \to \CohLoc$, such that $\Idl(B \stackrel{f}{\rightarrow} B') = (\Idl(B') \stackrel{f^{-1}}{\rightarrow}\Idl(B)$). A similar computation as with $\Ult(-)$, but with fewer conditions,  shows that the pre-image morphism $f^{-1}$ is well defined.
...
On the one hand, Lemma 7.1 shows us that $\DLat(B, \mathbb{2})$ lifts to $\prim(B)$. Indeed, $\Spec(B) = \Frm(\Idl(B), \mathbb{2})$, which gives us completely prime filters of $\Idl(B)$, and these correspond precisely to $\prim(B)$ [Joh 3.4]. 
...
Consider the cover $D(b) \subseteq \bigcup_I D(a_i)$. This is equivalent to saying there is no prime ideal $I$ containing all $a_i$ and $b$ (\textcolor{red}{TODO} Might need a Zorns lemma argument, check this later), which is equivalent to saying $1 \in (\{a_i\} \cup \{b\})$, i.e. the ideal generated by $a_i$ and $b$ is the entire spectrum. By downwards closure and closure under finite meets, there exists a finite index $m \in \mathbb{N}$ such that $1 \leq \bigvee_{i=0}^m a_i$ from which it follows $D(b) \subseteq \bigcup_{i = 0}^m D(a_i)$. Thus all $D(b)$ are compact and so is $\Spec(B)$.
...
1.(b) is only slightly more complicated. We start by remarking that $\mathbb{S} \notin \Stone$, so to pass the schizophrenic object downwards, we will want to Stonefy it via the Stone space reflection functor $\Stone(-)$, which is also called the Stoneification, i.e. the left adjoint to the forgetful functor (\textcolor{red}{TODO} find source to show this exists).

  By the dual equivalence we get the corresponding Boolean algebra reflection functor $\Bool(-)$, or Booleanification.
 
 To understand the Stoneification, we may first want to Booleanify the lattice of compact open subsets of a space $X$, which we will call $A$. Let $A^*$ be the set of complements in $\mathcal{P}(X)$ of members of $A$. Then the patch topology, which has as a base $C = \{U \cap V \lvert U \in A, V \in A^*\}$ , will be a Stone space (notice that we are taking the Boolean completion $B$ of $A$ and then taking its spectrum) [Joh].
...
The argument however works the same way as it would if $\mathbb{S}$ was in $\Stone$: $\CohTop(X, \mathbb{S})$ lifts to $\clop(X)$ if $X$ is a Stone space, since coherent maps into $\mathbb{S}$ are uniquely determined by the pre-image of $\{1\}$, which are simply the compact open subsets of a compact Hausdorff space, which are its  clopens. 
...
Now we can revisit the finite adjunction from the beginning of this section. Any finite Stone space is discrete, since Hausdorffness forces singletons to be clopen (we give singletons by an intersection of all point-separating clopens, of which there are finitely many, so that singletons are themselves clopen). This shows that $\FinStone = \FinSet$.

In fact we may even extend $\FinSet^{\op} \rightleftarrows \Boolf$ to Stone via $\Ind$ and $\Pro$ categories, as we get the following equivalence:
\begin{align*}
	\Bool \cong \Ind(\Boolf) \cong \Ind(\FinSet^{\op}) \cong  \Pro(\FinSet)^{\op}
\end{align*}
which means we can identify $\Stone$ spaces with profinite sets. For further reading check $Johnstone$ Chapter bla bla (\textcolor{red}{TODO} sources yeah).
...
That is to say a space is compactly generated if all its continuous functions are continuous on compact subspaces. Note that the domain in the definition of arbitrary compact space $Y$ could be restricted to compact subsets $K \subseteq X$ since images of compact spaces by continuous maps $f(Y) \subseteq X$ are homeomorphic to compact subspaces  $K \subseteq X$, so in particular, $f$ factors through the inclusion $\iota: K \hookrightarrow X$. 

This is the general definition that one might find in other textbooks, but for our purposes we restrict the definition of $k$-continuity to compact Hausdorff spaces, and refer to ${\kTop}$ as the category of compactly generated Hausdorff spaces, whose morphisms are the continuous functions between them, making it a full subcategory of $\Top$, and in particular, of $\Haus \subseteq \Top$. 
...
That is, given the set of inclusions $(K \stackrel{t_i}{\hookrightarrow} X)_{i \in I}$ of  compact subspaces $K\subseteq X$, we give $X$ the finest topology making all $t_i$ continuous. 
...
That is, if $X$ is not already Kelleyfied, we add opens $U = f^{-1}(V)$, for functions $f$ such that  $t_i^{-1}(U) \subseteq K$ is open  but $U \subseteq X$ is not. This is the universality condition, since commutativity must be satisfied for arbitrary function $f$. But this is our original statement: we want a topology on $X$ such that $f$ is continuous if and only if $f \circ t_i$ is continuous. 

Thus we can understand a $k$-space as a colimit of compact Hausdorff spaces, or specifically, $X \in \kTop$ if and only if $X = \colim_{K \subseteq X \text{ compact}}K$.

%\textcolor{red}{Question}: are compact subspaces in $X$ compact if and only if they are compact in Kelley($X$)? No.

We may otherwise view the \emph{Kelleyfication} as the right adjoint $k(-)$ to the forgetful functor. As $\kTop$ is a full subcategory of $\Haus$, this puts us in the setting of a coreflective subcategory of $\Haus \subseteq \Top$. 

Another way to think about $\kTop$ is as the coreflective hull of $\kHaus$ in $\Haus$. That is to say, the underlying functor $\kHaus \hookrightarrow \Haus$ does not have a right adjoint, since $\kHaus$ is not closed under colimits. However taking the intersection of all coreflective subcategories of $\Haus$ which contain $\kHaus$, noting that the inclusion functors are left adjoint so they preserves all colimits, and that the category generated by colimits of $\kHaus$ is itself coreflective, we obtain precisely the category generated under colimits of $\kHaus$.

Note that products are given as the Kelleyfications of topological  products, and furthermore, that for all locally compact Hausdorff spaces $X$, we have $X \times_k Y = X \times Y$ for all $Y \in \kTop$, since we can compute limits in $\Haus$ and pass back to $\kTop$ by the right adjoint, which preserves limits. One only needs to check that $X \in \kTop$

It is important that our category admits function spaces, or in other words, that we are in a closed category (i.e. all exponentiables exist). In the following we will show why $\kTop$ is a closed category. This will indeed give us intuition of why we have even defined $\kTop$ the way we have, as $\Haus $ is not a closed category.  
...
However in $\kTop$ function spaces are given as Kelleyfications of the set $\Top(X,Y)$ with the compact-open topology. (\textcolor{blue}{TODO} why?).
...
(\textcolor{red}{ToDO} Show that $\kTop$ admits function spaces )
...
\end{align*}
 ...
 For this example we assume familiarity with Galois theory. That is we assume the reader understands field extensions, basic group theory with respect to orbits and stabilizers, and we interpret a $\GSet$ to be a set equipped with a left group action, that is, a group homomorphism $\theta: G \to \Aut(S)$. Our main reference text shall be $\emph{Szamuely}$, though we also recommend $\emph{Bosch}$, for the non-categorical approach.
...
\begin{enumerate}
	...
	\item Ask Georg how to differentiate between $k$-Alg in terms of kelleyfied complex algebras and $k$-Alg in terms of algebras over a field $k$. We know that $\kAlg \subseteq \CAlgk$ as subcategory (not full), should this be reflected in the notation?
	...
	\item $X \times_k Y = X \times Y$ for $X$ locally compact, $Y \in \kTop$
	...
	\item why  is $C^*(X) \subseteq C(X)$
	\item $B \cap A \neq \emptyset $ for all $B \in F$ (filter) implies $B \cap A \in F$
	\item Why is $\kTop$ a monotopological category? Why does it have function spaces?
