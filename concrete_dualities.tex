...
% the initial topology making the inclusion $S(A) \hookrightarrow \mathbb{C}^A$ continuous, which is just 
 the initial topology making all evaluation maps $\Hom_k(A, \mathbb{C}_a) \stackrel{\ev_{A,a}}{\rightarrow} [\mathbb{C}_c]$ continuous. Indeed this is equivalent to the initial topology making the inclusion $S(A) \hookrightarrow \mathbb{C}^A$ continuous, from which Hausdorffness is clear, since the product of Hausdorff spaces is Hausdorff, and the subspace of a Hausdorff space is Hausdorff. 
...
A useful consequence of this perspective is that recognizing a potential schizophrenic object can serve as the primary step toward understanding a duality. In each case, the schizophrenic object provides the most intuitive point of entry, and identifying it is typically far easier than attempting to describe the adjunction directly. 

The level of detail varied according to feasibility, and we did not attempt to develop the full surrounding theory within each subfield. However what we did show was that once such an object is identified, the dual adjunction — with its associated structure — follows, making it unnecessary to rebuild the theory from the ground up for each new instance. In this sense, knowing how concrete dualities arise allows one to see familiar constructions not as isolated coincidences but as manifestations of a single underlying mechanism.
%\begin{enumerate}
