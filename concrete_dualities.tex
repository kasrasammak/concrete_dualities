...
The first equality comes from the fact that a boolean algebra is a special case of a frame, so that morphisms in $\Bool(A, \mathbb{2})$ are left exact and colimit preserving, which means that a morphism is uniquely determined by which $a \in A$ is sent to 1 (\textcolor{red}{TODO}: show this).
