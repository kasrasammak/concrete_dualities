...
\end{align*}
where the first isomorphism is easily shown by defining a map which combines components and its inverse which separates them. Th
...
That is to say ${\{L|M|k\} }^{\op}$ is codirected since $L|k$ is an initial object of the poset, and similarly ${\{H \leq \Gal(L/k)\}}$ is directed  ${\{L|M|k\} }^{\op}$ since $\Gal(L/k)$ is terminal, corresponding to the full subgroup $\Gal(L/k)$. 
...
Since $G$ is a group of autormorphisms on $k^s$, a $G$-action on maps
