...
A functor $\mathcal{A} \stackrel{G}{\to}\mathcal{B}$ is called \textbf{topological} if every $G$-structured source has a unique $G$-initial lift. 
 ...
 Notice here again that our intuition of such a category should come from topology, which is even reflected in its name, namely, from the construct $(\Top, U)$,  over which every source lifts initially, since the arbitrary intersection of topologies is a topology. That is, we are taking the intersection of all topologies on $A$ such that all $(GA \stackrel{f_i}{\to} GA_i)_I$ are continuous.
 
  That means there is a systematic way to choose open sets of $GA$. In other words,  a subset $S \subset GA$ lifts to an open set in $A$ if and only if $S$ equals the preimage $Gf_i^{-1}(G(U))$ for some  $f_i \in (GA \stackrel{f_i}{\to} GA_i)_I$ and $U \in  \tau_{A_i}$.
...
Now we want to see that a category which admits an internal hom-functor induces a dual adjunction. Let $\concA$ admit an internal hom-functor $H$, and let there be a concrete category $\concB$ and a concrete functor $\lvert - \lvert : \mathcal{B} \to \mathcal{A}$ such that $\HomB(B, C) \hookrightarrow \HomA(\lvert B \lvert, \lvert C \lvert)$ lift to $\mathcal{A}$-morphisms\footnote{$\HomB_\mathcal{A}(-,-)$ is notation for the hom set in $\mathcal{B}$ as $\mathcal{A}$-object} $\gamma_{B,C}: \HomB_\mathcal{A}(B,C) \to H(\lvert B \lvert, \lvert C\lvert)$, which can be done in a monotopological category by lifting initially.
