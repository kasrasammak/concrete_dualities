...
But we can see in the background we actually send a boolean algebra to its frame of ideals, and consider the points of that frame, which correspond to completely prime filters. However, since were in  the opposite category, we consider that the completely prime ideals $I$ of $\Idl(A)$ correspond to prime ideals $P$ of $A$ by the bijective assignments $P \mapsto \{I \in \Idl(A) \lvert I \cap P \neq \emptyset\}$ and $F \mapsto \{a \in A \lvert \downarrow (b) \in F \}$ [Joh] so that the prime ideals of $A$ correspond to the spectrum.

In a word, the points of the locale of ideals of $A$, or the \emph{spectrum} of $A$, are precisely its set of prime ideals.  

But the second map we also have, given by the composed duality $\CohLoc \rightleftarrows \CohTop$, since we have already identified the initial lift of function space into the Sierpinski space to the category frames, so we need only compose it. 

Consider that morphisms in $\Stone$ are not merely continuous maps between $\Stone$ spaces, but the maps must also be coherent, i.e., they must preserve compact opens. In a stone space, compact opens are precisely clopens, so $\Stone$ morphisms are precisely the continuous morphisms between stone spaces, whose preimage preserves clopen subsets. But Stone space morphisms are given by precisely where they send the subspace $\{1\} \subset \mathbb{S}$ to which are precisely the clopen sets of $X$. 
