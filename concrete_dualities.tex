...
\end{enumerate}
Notice that if $B$ is spatial, which implies the existence of a topological space $X = \pt(B)$ such that $B = \Omega(X)$, then the finite elements are precisely the compact subsets of $X$. 
\begin{definition}
...
Since $K(B)$ is a sub join-semilattice of $B$ [Joh], the second condition is equivalent to $1 \in K(B)$ and $K(B)$ is closed under finite intersections.
...
So we may define a locale map $B \stackrel{f}{\to} A$ between coherent locales to be  \emph{\textbf{coherent}} if $f^*$ maps $K(A)$ to $K(B)$.

Unlike the other examples, that means the restriction of $\Spat$ to $\CohLoc$ is not full.


However, we have shown that we do get a dual equivalence
...
In particular, this is also a natural dual adjunction. Notice
Now since we want to compose these dualities $\CohTop^{\op} \rightleftarrows \CohLoc^{\op} \rightleftarrows \DLat$, a natural candidate for schizophrenic object might be $\Omega(\mathbb{S}) = \bot - b - \top$, which we shall call $ \mathbb{S}_{\Lat}$, whose elements are all finite, so that  $K(\Omega(\mathbb{S}))$ leaves $ \Omega(\mathbb{S})$ unchanged.
...
But we aren't done yet, we will now want to restrict the composition.
...
The answer to the first question we already gave in the leading example, as the set of points of a Boolean algebra. Remember at the outset we actually defined the adjunction by the identification of ultrafilters of a finite set $X$ with points of a finite boolean algebra, and points of any lattice can be identified with its prime ideals.

But we can see in the background we actually send a boolean algebra $B$ to its frame of ideals $\Idl(B)$, and consider the points of that frame, which correspond to completely prime filters of $\Idl(B)$. These correspond to prime filters $P$ of $B$ by the bijective assignments $P \mapsto \{I \in \Idl(B) \lvert I \cap P \neq \emptyset\}$ and $F \mapsto \{b \in B \lvert \downarrow (b) \in F \}$ [Joh] so that the prime ideals of $B$ correspond to the spectrum.

In a word, the points of the locale of ideals of $A$, or the \emph{spectrum} of $A$, are precisely its set of prime ideals.  

Now such a lift is initial by the same argumentation as in the leading example: we simply take the initial topology such that all evaluation maps $\Bool(B, \mathbb{2}) \to \mathbb{2}$ are continuous, which have as a subbasis  $\{\ \{p \in \Bool(B,\mathbb{2}) \lvert p(b) = 1\}\  \lvert \ b \in B \ \}$.
...
Remember that each restriction is simply the free generator or left adjoint of the forgetful functor, whose underlying functors into subcategories of $\Top$ compose with the underlying functor into $\Top$.
 ...
 We must be careful here, because now the boolean completion of $\mathbb{S}$ in $\mathcal{P}(\{0,1\})$ is simply $\mathbb{2}_s$ or the two point discrete space, since by booleanifying we must generate the complement of $\{1\}$.
 
 This shows us that the left adjoint of the Stone duality is given as an initial lift of $\Stone(-, \mathbb{2}_s)$. 
