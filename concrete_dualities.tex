...
In this thesis we normally use concrete category to mean construct, since we do not explicitly consider any other type of concrete category.
...
If $(\mathcal{A}, U)$ is a topological concrete category admitting function spaces, we have, for all $A \in \mathcal{A}$ the source $(\top \stackrel{\t x}{\rightarrow} A)_{x \in [A]}$. (\textcolor{red}{TODO} Show this) where $\top$ is the terminal object. That is to say $\HomA(\top, A) = [A]$. Therefore, we would get, for any set map $[f]:[A'] \to [A]$ a unique lifting diagram
...
\end{tikzcd}\]
In other words, we would be able to lift arbitrary set maps to $\mathcal{A}$-morphisms, which, for non-trivial categories  is almost never true.
...
Now we can revisit the finite adjunction from the beginning of this section. Any finite Stone space is discrete, since Hausdorffness forces singletons to be clopen (we give singletons by an intersection of all point-separating clopens, of which there are finitely many, so that singletons are themselves clopen). This shows that $\FinStone = \FinSet$, and it is thus clear that Stone restricts to the finite duality from 7.a.

In fact we may  extend\footnote{For further reading look at Johnstone's last chapter. As we said at the beginning of the section, had we constructed Stone this way, we would have bypassed elucidating the nature of the Stone duality and had to introduce abstraction which is unnecessary for our purposes.} $\FinSet^{\op} \rightleftarrows \Boolf$ to Stone via $\Ind$ and $\Pro$ categories, as we get the following equivalence:
\begin{align*}
	\Stone \cong \Bool^{\op} \cong \Ind(\Boolf)^{\op} \cong \Ind(\FinSet^{\op})^{\op} \cong  \Pro(\FinSet).
\end{align*}

For further reading check $Johnstone$ Chapter bla bla (\textcolor{red}{TODO} sources yeah).
...
Note that products are given as the Kelleyfications of topological  products. Furthermore,  any locally compact Hausdorff space $X$ is in $\kTop$, so we have $X \times_k Y = X \times Y$ for all $Y \in \kTop$, since $k(-)$ preserves limits. 
...
which, by remarks of \textbf{4.c}, is clearly not natural.
...
%	\item ask Georg if pointwise order preserving evaluation morphism into $\mathbb{2}$ preserves finite limits and colimits by some pointwise argument in the presheaf category
	...
	\item Why is $\kTop$ does $\kTop$ admit function spaces? Why doesn't $\Haus$?
