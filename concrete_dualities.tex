...
In this part we assume basic knowledge about topological spaces and frames. We will only use the definitions of objects and morphisms in $\Top$ and $\Frm$.  Our reference text when it comes to any discussion about posets, lattices, frames, or locales, is \emph{Johnstone}'s \emph{Stone Spaces} \cite{zbMATH03940199}. 
...
Since $K(B)$ is a sub join-semilattice of $B$ (closed under finite joins) \cite{zbMATH03940199}, the second condition is equivalent to $1 \in K(B)$ and $K(B)$ is closed under finite meets.
...
Note that Stone spaces are compact and totally disconnected \cite{zbMATH03940199}making them a subcategory of $\kHaus$. Moreover, if $X$ is Hausdorff, then $K(\Omega(X)) = \clop(X)$, since compact sets are exactly the closed sets of a compact Hausdorff space.
...
1.(b) is only slightly more complicated. We start by remarking that $\mathbb{S} \notin \Stone$, so to pass the schizophrenic object downwards, we will want to Stonefy it via the Stone space reflection functor $\Stone(-)$, which is also called the Stoneification, i.e. the left adjoint to the forgetful functor (\textcolor{red}{TODO} find source to show this exists).
 ...
 To understand the Stoneification, we may first want to Booleanify the lattice of compact open subsets of a space $X$, which we will call $A$. Let $A^*$ be the set of complements in $\mathcal{P}(X)$ of members of $A$. Then the patch topology, which has as a base $C = \{U \cap V \lvert U \in A, V \in A^*\}$ , will be a Stone space (notice that we are taking the Boolean completion $B$ of $A$ and then taking its spectrum) \cite{zbMATH03940199}.
...
In fact we may  extend\footnote{For further reading look at Chapter VI. in \emph{Johnstone's Stone Spaces}. As we said at the beginning of the section, had we constructed Stone this way, we would have bypassed elucidating the nature of the Stone duality and had to introduce abstraction which is unnecessary for our purposes.} $\FinSet^{\op} \rightleftarrows \Boolf$ to Stone via $\Ind$ and $\Pro$ categories, as we get the following equivalence:
\begin{align*}
